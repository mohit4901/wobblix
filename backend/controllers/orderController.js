...
const placeOrderRazorpay = async (req, res) => {
  try {
    ...
    const freeCount = Math.floor(eligiblePrices.length / 5);
    let b4g1Discount = 0;
    for (let i = 0; i < freeCount; i++) {
      b4g1Discount += eligiblePrices[i];
    }

    // Verify coupon on server side to prevent tampering (Exclusivity: only if B4G1 is not active)
    let couponDiscountAmount = 0;
    if (couponCode === 'NEW10' && b4g1Discount === 0) {
    ...
    const finalVerifiedAmount = Math.max(0, actualSubtotal - couponDiscountAmount + shippingFee);
...
const verifyCoupon = async (req, res) => {
    try {
        ...
        // Check if user is a newcomer (up to 5 orders)
        const previousOrders = await orderModel.find({ userId, payment: true });
        if (previousOrders.length >= 5) {
            return res.json({ success: false, message: "Coupon valid for first 5 orders only" });
        }

        res.json({ success: true, discount: 15, message: "15% Discount Applied!" });
