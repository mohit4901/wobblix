...
const TrousersCollection = () => {
  const { products } = useContext(ShopContext)
  const [trousers, setTrousers] = useState([])

  useEffect(() => {
    // Filter for Trousers
    const filtered = products.filter(item => item.subCategory === "Trousers")
    // Sort by date descending
    filtered.sort((a, b) => b.date - a.date)
    setTrousers(filtered.slice(0, 5))
  }, [products])

  if (trousers.length === 0) return null;

  return (
    <div className="w-full py-20 px-4 sm:px-10 lg:px-16 bg-[#edece8] border-t border-gray-200">
      <div className="text-center mb-14">
        <Title text1="TROUSER" text2="DROPS" />
        <p className="w-3/4 m-auto text-xs sm:text-sm text-gray-500 font-bold tracking-widest mt-4 uppercase">
          Streetwear essential utility. Premium heavyweight comfort and custom silhouettes.
        </p>
      </div>
...
export default TrousersCollection
