...
const ShopContextProvider = (props) => {
  ...
  const getProductsData = async () => {
    try {
      ...
      const sortedLocal = [...(localProducts || [])].sort((a, b) => b.date - a.date);
      setProducts(sortedLocal);
    } finally {
      setLoading(false);
    }
  };
  ...
  const getB4G1Discount = () => {
    ...
    const freeCount = Math.floor(eligibleItems.length / 5);
  ...
  // GET COUPON DISCOUNT (15% off up to 3 tshirts, mutually exclusive with B4G1)
  const getCouponDiscount = () => {
    if (!couponCode || couponCode !== 'NEW10') return 0;
    
    // If B4G1 promo is active, coupon cannot be applied
    const b4g1Discount = getB4G1Discount();
    if (b4g1Discount > 0) {
      return 0;
    }

    // 15% off up to 3 tshirts
    let tshirtPrices = [];
    for (const id in cartItems) {
      const product = products.find(p => p._id === id);
      if (!product) continue;
    ...
    // Sort descending to discount the 3 most expensive tshirts
    tshirtPrices.sort((a, b) => b - a);
    const eligibleTshirts = tshirtPrices.slice(0, 3);
    const eligibleSum = eligibleTshirts.reduce((sum, price) => sum + price, 0);

    return Math.round((eligibleSum * 15) / 100);
  };
  ...
  const applyCoupon = async (code) => {
    ...
    if (getB4G1Discount() > 0) {
        toast.error("NEW10 coupon cannot be combined with Buy 4 Get 1 Free promotion.");
        return;
    }
    try {
  ...
  // Auto-remove coupon if B4G1 becomes active
  useEffect(() => {
    if (products.length > 0 && couponCode && getB4G1Discount() > 0) {
      setCouponCode("");
      setDiscount(0);
      toast.info("Coupon removed: Buy 4 Get 1 Free promo is active.");
    }
  }, [cartItems, products, couponCode]);
