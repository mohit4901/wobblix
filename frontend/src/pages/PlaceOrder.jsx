...
const PlaceOrder = () => {
    ...
    const { navigate, backendUrl, token, cartItems, setCartItems, getCartAmount, delivery_fee, products, discount, applyCoupon, couponCode, getB4G1Discount, getCouponDiscount } = useContext(ShopContext);
            ...
            <div className='lg:w-[450px] space-y-12'>
                ...
                <div className="bg-black text-white p-10 shadow-2xl">
                    ...
                    <div className='mt-8 pt-8 border-t border-gray-800'>
                        ...
                        {/* Interactive Available Offers */}
                        <div className="mt-8 space-y-4">
                            <p className="text-[10px] font-black uppercase tracking-widest text-gray-400">Available Offers</p>
                            
                            {/* NEW10 Card */}
                            <div 
                                onClick={() => {
                                    setCouponInput("NEW10");
                                    applyCoupon("NEW10");
                                }}
                                className={`border p-4 cursor-pointer transition-all duration-300 flex items-center justify-between ${couponCode === 'NEW10' ? 'border-green-500 bg-green-950/20' : 'border-dashed border-gray-700 hover:border-white bg-gray-900/50'}`}
                            >
                                <div>
                                    <div className="flex items-center gap-2">
                                        <span className="text-[10px] font-black tracking-wider bg-white text-black px-2 py-0.5">NEW10</span>
                                        <span className="text-[10px] font-bold text-gray-300">15% OFF</span>
                                    </div>
                                    <p className="text-[10px] text-gray-400 mt-2 uppercase tracking-wider font-bold">Valid on your first 5 orders</p>
                                </div>
                                <span className={`text-[10px] font-black uppercase tracking-widest ${couponCode === 'NEW10' ? 'text-green-500 font-black' : 'text-white underline'}`}>
                                    {couponCode === 'NEW10' ? 'APPLIED' : 'APPLY'}
                                </span>
                            </div>

                            {/* B4G1 Card */}
                            <div className="border border-dashed border-gray-700 p-4 bg-gray-900/50 flex items-center justify-between">
                                <div>
                                    <div className="flex items-center gap-2">
                                        <span className="text-[10px] font-black tracking-wider bg-red-600 text-white px-2 py-0.5">B4G1</span>
                                        <span className="text-[10px] font-bold text-gray-300">AUTO-APPLIED</span>
                                    </div>
                                    <p className="text-[10px] text-gray-400 mt-2 uppercase tracking-wider font-bold">Buy 4, get 5th cheapest item free</p>
                                </div>
                                <span className={`text-[10px] font-black uppercase tracking-widest ${getB4G1Discount && getB4G1Discount() > 0 ? 'text-green-400' : 'text-gray-500'}`}>
                                    {getB4G1Discount && getB4G1Discount() > 0 ? 'ACTIVE' : 'INACTIVE'}
                                </span>
                            </div>
                        </div>
                    </div>
